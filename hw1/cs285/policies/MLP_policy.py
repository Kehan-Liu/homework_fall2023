...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def update(self, observations, actions):
        ...
        action_mean = self.forward(observations)
        action_stdinv = torch.exp(-self.logstd)
        losses = 1 / 2 * torch.sum(
            action_stdinv * (action_mean - actions) ** 2, dim=1
        )
        loss = 1 / 2 * torch.sum(self.logstd) + losses.mean()
        loss.backward()
        self.optimizer.step()
        return {

...
class PGAgent(nn.Module):
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [
                self._discounted_reward_to_go(reward_traj) for reward_traj in rewards
            ]
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            obst = ptu.from_numpy(obs)
            values = ptu.to_numpy(self.critic(obst))
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discounted_return = np.sum(rewards * (self.gamma ** np.arange(T)))
        return np.full_like(rewards, fill_value=discounted_return)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        return (rewards * (self.gamma ** np.arange(T)))[::-1].cumsum()[::-1] / (
            self.gamma ** np.arange(T)
        )
